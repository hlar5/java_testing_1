...
public class Mino {
    ...
    public void checkMovementCollision(){
        ...
        for(int i = 0; i < b.length; i++){
            if(b[i].x == GameFrame.left_x){
                leftCollision = true;
            }
        }

        for(int i = 0; i < b.length; i++){
            if(b[i].x + Block.SIZE == GameFrame.right_x){
                rightCollision = true;
            }
        }

        for(int i = 0; i < b.length; i++){
            if(b[i].y + Block.SIZE == GameFrame.bottom_y){
    ...
    public void checkRotationCollision(){
        ...
        for(int i = 0; i < b.length; i++){
            if(tempB[i].x < GameFrame.left_x){
                leftCollision = true;
            }
        }

        for(int i = 0; i < b.length; i++){
            if(tempB[i].x + Block.SIZE > GameFrame.right_x){
                rightCollision = true;
            }
        }

        for(int i = 0; i < b.length; i++){
            if(tempB[i].y + Block.SIZE > GameFrame.bottom_y){
                bottomCollision = true;
            }
        }
    ...
    private void checkStaticBlockCollision(){
        for(int i = 0; i < GameFrame.staticBlocks.size(); i++){
            int targetX = GameFrame.staticBlocks.get(i).x;
            int targetY = GameFrame.staticBlocks.get(i).y;
    ...
    public void update(){
        ...
        if(bottomCollision){
            ...
            autoDropCounter++;
            if(autoDropCounter == GameFrame.dropInterval){
